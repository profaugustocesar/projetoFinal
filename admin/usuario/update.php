<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (isset($_POST['txtEmail']) and !empty($_POST['txtEmail'])) {
            ...
            $select = $pdo->prepare('SELECT * FROM usuario WHERE email = :pEmail and id <> :pId');
        ...
        if (count($erros) == 0) {
            ...
            try {
                ...
                if ($alterouSenha) {
                    $insert = $pdo->prepare('UPDATE usuario SET nome=:pNome, email=:pEmail, senha=:pSenha, salt=:pSalt, nivelAcesso=:pNivelAcesso WHERE id = :pId');
                    ...
                    $insert->bindValue(':pId',$txtId);
                } else {
                    $insert = $pdo->prepare('UPDATE usuario SET nome=:pNome, email=:pEmail, nivelAcesso=:pNivelAcesso WHERE id = :pId');
                    $insert->bindValue(':pNome',$txtNome);
                    $insert->bindValue(':pEmail',$txtEmail);
                    $insert->bindValue(':pNivelAcesso',$txtNivelAcesso);
                    $insert->bindValue(':pId',$txtId);
